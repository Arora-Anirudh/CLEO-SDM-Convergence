...
def bootstrap_ensemble_mean_l1(
    member_distributions_gm3_per_ln_radius: np.ndarray,
    analytical_gm3_per_ln_radius: np.ndarray,
    edges_um: np.ndarray,
    *,
    ...
    """Bootstrap L1 of an ensemble-mean distribution."""
    members = np.asarray(member_distributions_gm3_per_ln_radius, dtype=float)
    analytical = np.asarray(analytical_gm3_per_ln_radius, dtype=float)
    edges_um = np.asarray(edges_um, dtype=float)
    if members.ndim != 2 or members.shape[0] < 2:
        raise ValueError("at least two member distributions are required")
    if members.shape[1:] != analytical.shape:
        raise ValueError("member and analytical bin counts must match")
    if bootstrap_resamples < 1:
        raise ValueError("bootstrap_resamples must be positive")
    if not 0.0 < confidence_level < 1.0:
    ...
    indices = generator.integers(
        0,
        members.shape[0],
        size=(bootstrap_resamples, members.shape[0]),
    )
    counts = np.stack(
        [np.sum(indices == member, axis=1) for member in range(members.shape[0])],
        axis=1,
    )
    means = counts.astype(float) @ members / members.shape[0]
    delta_ln_radius = np.diff(np.log(edges_um))
    denominator = float(np.sum(np.abs(analytical) * delta_ln_radius))
    if denominator <= 0:
        raise ValueError("analytical L1 denominator must be positive")
    bootstrap_l1 = np.sum(np.abs(means - analytical) * delta_ln_radius, axis=1) / denominator
    alpha = 1.0 - confidence_level
    ci_low, ci_high = np.quantile(
        bootstrap_l1,
        [alpha / 2.0, 1.0 - alpha / 2.0],
    )
    observed = ensemble_mean_fixed_bin_relative_l1(
        members,
        analytical,
        edges_um,
    )
...
def independent_bootstrap_l1_difference(
    member_distributions_gm3_per_ln_radius: np.ndarray,
    reference_member_distributions_gm3_per_ln_radius: np.ndarray,
    analytical_gm3_per_ln_radius: np.ndarray,
    edges_um: np.ndarray,
    *,
    ...
    """Bootstrap the L1 difference between two independent ensembles."""
    candidate = np.asarray(member_distributions_gm3_per_ln_radius, dtype=float)
    reference = np.asarray(
        reference_member_distributions_gm3_per_ln_radius,
        dtype=float,
    ...
    if candidate.ndim != 2 or reference.ndim != 2:
        raise ValueError("member distributions must have shape (members, bins)")
    if candidate.shape[0] < 2 or reference.shape[0] < 2:
        raise ValueError("at least two members are required in each ensemble")
    if candidate.shape[1:] != reference.shape[1:] or candidate.shape[1:] != analytical.shape:
        raise ValueError("candidate, reference and analytical bin counts must match")
    if bootstrap_resamples < 1:
        raise ValueError("bootstrap_resamples must be positive")
    if not 0.0 < confidence_level < 1.0:
        raise ValueError("confidence_level must lie strictly between zero and one")
    ...
    candidate_indices = generator.integers(
        0,
        candidate.shape[0],
        size=(bootstrap_resamples, candidate.shape[0]),
    )
    reference_indices = generator.integers(
        0,
        reference.shape[0],
        size=(bootstrap_resamples, reference.shape[0]),
    )
    candidate_counts = np.stack(
        [np.sum(candidate_indices == member, axis=1) for member in range(candidate.shape[0])],
        axis=1,
    )
    reference_counts = np.stack(
        [np.sum(reference_indices == member, axis=1) for member in range(reference.shape[0])],
        axis=1,
    )
    candidate_means = candidate_counts.astype(float) @ candidate / candidate.shape[0]
    reference_means = reference_counts.astype(float) @ reference / reference.shape[0]
    delta_ln_radius = np.diff(np.log(edges_um))
    denominator = float(np.sum(np.abs(analytical) * delta_ln_radius))
    if denominator <= 0:
        raise ValueError("analytical L1 denominator must be positive")
    ...
    differences = candidate_l1 - reference_l1
    alpha = 1.0 - confidence_level
    ci_low, ci_high = np.quantile(
        differences,
        [alpha / 2.0, 1.0 - alpha / 2.0],
